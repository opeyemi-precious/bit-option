...
;; Create Option - Deploy new options contract
  ...
  (let (
    ...
    (asserts!
      (and
        ...
        (<= (- expiry stacks-block-height) u5200)
      )
      ERR_INVALID_EXPIRY
    )
    ;; Max 1 month
    ...
    (map-set user-balances tx-sender
      (merge user-balance { locked-collateral: (+ (get locked-collateral user-balance) required-collateral) })
    )
    ...
    (if (is-eq (get option-type option) "CALL")
      ...
      (if (> current-price (get strike-price option))
        (let ((profit (- current-price (get strike-price option))))
          ...
          (map-set options option-id (merge option { status: "EXERCISED" }))
          (ok true)
        )
